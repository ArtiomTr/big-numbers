...
bool testSingleNumber() {
    BigInt<uint8_t> one({1}, 0);
    BigInt<uint8_t> two({2}, 0);

    BigInt<uint8_t> result = one + two;

    return testBigInt(result, BigInt({3}, 0));
}
...
bool testShortNumbers() {
    ...
    return testBigInt(result, BigInt({0b00011001, 0b10000001}, 0)); // 33049
}
...
bool testLongerNumbers() {
    ...
    // 798215633405
    return testBigInt(result, BigInt({0b11111101, 0b11111001, 0b01011011, 0b11011001, 0b10111001}, 0));
}
...
bool testDiffSizedNumbers() {
    ...
    return testBigInt(result, BigInt({0b10111001, 0b10001000}, 0)); // 35001
}

// Testing sum carry inside one cell.
bool testCarry() {
    BigInt<uint8_t> one({0b00000001}, 0); // 1
    BigInt<uint8_t> two({0b00000001}, 0); // 1

    BigInt<uint8_t> result = one + two;

    return testBigInt(result, BigInt({0b00000010}, 0)); // 2
}

// Testing sum carry between two cells.
bool testLongCarry() {
    BigInt<uint8_t> one({0b10000000, 0b00000000}, 0); // 128
    BigInt<uint8_t> two({0b10000000, 0b00000000}, 0); // 128

    BigInt<uint8_t> result = one + two;

    return testBigInt(result, BigInt({0b00000000, 0b00000001}, 0)); // 256
}

// Testing cell creation when carrying on last cell.
bool testCellCreation() {
    BigInt<uint8_t> one({0b10000000}, 0); // 128
    BigInt<uint8_t> two({0b10000000}, 0); // 128
...
bool testFullCellCarry() {
    BigInt<uint8_t> one({0b10000000, 0b11111111, 0b00000000}, 0); // 65408
    BigInt<uint8_t> two({0b10000000, 0b11111111, 0b00000000}, 0); // 65408

    BigInt<uint8_t> result = one + two;

    return testBigInt(result, BigInt({0b00000000, 0b11111111, 0b00000001}, 0)); // 130816
}
