...
bool testSingleNumber() {
    big_int<uint8_t> one({2}, 0);
    big_int<uint8_t> two({1}, 0);

    big_int<uint8_t> result = one - two;

    return test_big_int(result, big_int({1}, 0));
}
...
bool testShortNumbers() {
    ...
    return test_big_int(result, big_int({0b11111001, 0b10000000}, 1)); // -32519
}
...
bool testLongerNumbers() {
    ...
    // -370563098211
    return test_big_int(result, big_int({0b10011101, 0b11011001, 0b10110111, 0b10111000, 0b10101001}, 1));
}
...
bool testDiffSizedNumbers() {
    ...
    return test_big_int(result, big_int({0b10101001, 0b01110111}, 1)); // -34903
}
...
bool testEmptySubtraction() {
    ...
    return test_big_int(result, big_int({0}, 1));
}
